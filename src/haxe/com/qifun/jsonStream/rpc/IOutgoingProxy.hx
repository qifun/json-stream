...
@:autoBuild(com.qifun.jsonStream.rpc.IOutgoingProxy.OutgoingProxyGenerator.build())
...
class OutgoingProxyGenerator
{
        ...
        }:
        {
          ...
          var methodBody =
            macro this.outgoingRpc(
              com.qifun.jsonStream.rpc.IOutgoingProxy.OutgoingProxyRuntime.object1(
                $v{methodName},
  ...
  macro public static function build():Array<Field> return
  {
    for (pair in Context.getLocalClass().get().interfaces)
    {
      switch (pair)
      {
        case
        {
          t: _.get() =>
          {
            module: "com.qifun.jsonStream.rpc.IOutgoingProxy",
            name: "IOutgoingProxy",
          },
    ...
    throw "Cannot find IOutgoingProxy!";
  }
}
